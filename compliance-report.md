...
- Date: 2026-08-31 (refreshed after Slack interactive features: actions, summaries, App Home, unfurls)
...
| 2.5 | Minimum-permission scopes | Secrets & Credentials | AUTO | Yes | Slack manifest requests focused bot scopes and now adds capability scopes only when the matching admin feature is enabled (`app_mentions:read`, `channels:history`/`groups:history` for consented summaries, `links:read`/`links:write` for own-domain unfurls); Jira and MCP credentials are scoped in their respective flows. IT note: enabling thread summaries grants channel-history read — content is processed in memory only, never persisted. |
...
| 5.3 | Webhook signature validation | Network & Integration Security | AUTO | Yes | All three Slack endpoints (events, commands, interactions) verify the v0 HMAC over the raw body plus a 5-min replay window before parsing, and pin `api_app_id` fail-closed; interactive button values are additionally HMAC-signed server references with server-side ownership re-checks. Jira OAuth validates state. |
