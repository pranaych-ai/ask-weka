...
async def send_test(
    ...
    data = await svc.slack_api(
        "chat.postMessage",
        {"channel": target, "text": text, "blocks": svc.text_blocks(text)},
    )
    if not data.get("ok"):
        raise HTTPException(502, f"Slack rejected the message ({data.get('error', 'unknown')})")
...
async def put_prefs(
    ...
    unknown = set(req.prefs) - {k for k, m in svc.FEATURES.items() if m["user_optin"]}
    if unknown:
